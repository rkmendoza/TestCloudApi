Feature: As a user of riskIQ platform I want to Set Sinkhole Status
 ...
 @SetSinkholeStatus
  ...
  @SetSinkholeStatus
  Scenario: Check the response of Set Sinkhole Status with false param, and the visibility is public with the user not in the same organization, the response retrieve code 200 and check with json schema
    Given a created project with values
      | key                | value                  |
      | name               | @@namerandom           |
      | visibility         | public                 |
