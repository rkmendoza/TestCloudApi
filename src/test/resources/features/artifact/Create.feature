Feature: As a user of riskIQ platform I want to Create artifact
  ...
  @CreateArtifact
  Scenario: Check the response of Create artifact with an not exist GUID the response retrieve error message and code 400 error and check with json schema
    Given a created project with values
      | key          | value                  |
      | name         | @@namerandom           |
      | visibility   | public                 |
